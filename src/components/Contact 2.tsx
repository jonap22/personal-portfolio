...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!formRef.current || !publicKey || !serviceId || !templateId) return
    setStatus('sending')
    try {
          ...
          <AnimateIn delay={0.25}>
            <div className="flex flex-wrap gap-3 mb-14">
              <a
                ...
                className="inline-flex items-center gap-2 px-4 py-2.5 border border-[rgba(0,0,0,0.08)] rounded-xl bg-white/30 hover:bg-white/60 transition-all duration-200 text-sm text-muted hover:text-foreground"
              >
                <Mail size={14} />
            ...
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label
                    htmlFor="name"
                    className="block font-mono text-xs tracking-widest uppercase text-muted mb-2.5"
                  >
                    Name
                  </label>
                  <input
